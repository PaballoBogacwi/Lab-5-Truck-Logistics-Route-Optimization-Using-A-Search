...
# The graph is undirected, so each road appears in both cities' adjacency lists.
graph = {
...
# 3. Heuristic values to the goal node, Coligny
# These values are the traffic-per-city values provided in the practical.
heuristic = {
    "Disaneng": 70,
    "Mahikeng": 50,
    "Mmabatho": 55,
...
def get_cost(a, b, weights):
    """Return the cost of an undirected edge between a and b."""
    if (a, b) in weights:
        return weights[(a, b)]
    if (b, a) in weights:
        return weights[(b, a)]
...
def astar(graph, weights, heuristic, start, goal):
    """Perform A* Search and return path, visited order, and total path cost."""
    open_set = []
...
def simulate_path(path, visited_order, total_cost, delay=0):
    """Display the route followed by the truck."""
    if path is None:
        print("No path found.")
        return
